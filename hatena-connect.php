...
class hatena_connect {
	...
	function __construct() {
		...
		register_uninstall_hook( __FILE__, array( 'hatena_connect', 'hc_uninstall_hook' ) );
	...
	function hc_options_page() {
		if( isset( $_POST['sended'] ) ) {
			check_admin_referer('hc_options');
			$this->options['hatena_id'] = $_POST['hc_hatena_id'];
			$this->options['remove_more'] = ( isset( $_POST['hc_remove_more'] ) ) ? true : false;
		...
		<div class="wrap">
			...
			<form name="form" action="" method="post">
				<?php wp_nonce_field( 'hc_options' ); ?>
				<input type="hidden" name="sended" value="1">
				<table class="form-table">
					<tr>
						<th><?php _e( 'Hatena ID', 'hatena-connect' ); ?></th>
						<td><input id="hc_hatena_id" name="hc_hatena_id" class="regular-text code" type="text" value="<?php echo $this->options['hatena_id']; ?>" pattern="^[0-9A-Za-z-_]+$" title="<?php _e( 'alphanumeric, hyphen, underscore', 'hatena-connect' ); ?>"></td>
					</tr>
					<tr>
						<th><?php _e( 'Remove #more anchor', 'hatena-connect' ); ?></th>
						<td><label for="hc_remove_more"><input type="checkbox" name="hc_remove_more" id="hc_remove_more" value="1"<?php if( $this->options['remove_more'] ): ?> checked<?php endif; ?>> <?php _e( 'Remove more anchor from "Read More" links to prevent bookmarking duplicate content', 'hatena-connect' ); ?></label></td>
					</tr>
				</table>
